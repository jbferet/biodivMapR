...
`biodivMapR` v1. 
A unique function named `biodivMapR` runs the full workflow and produces a 
limited set of outputs.

## available spectral diversity metrics

`biodivMapR` allows computing and mapping various diversity metrics. 

- $\alpha$ diversity metrics include spectral `richness`, `shannon`, `simpson`, 
and `hill` index. The default Hill order is set to 1, corresponding to exp(shannon). 
Users can define a Hill number with the parameter `Hill_order`.
        
        
- $\beta$ diversity metrics include Bray-Curtis dissimilarity (`bray`), 
Bray-Curtis turnover (`brayturn`), Jaccard dissimilarity (`jaccard`), Jaccard 
turnover (`jaccardturn`), Simpson dissimilarity (`simpson_diss`), and Sorensen 
dissimilarity (`sorensen`). Please refer to the R package 
[`bioregion`](https://biorgeo.github.io/bioregion/articles/a3_pairwise_metrics.html#compute-pairwise-dissimilarity-metrics) 
for a definition of these metrics.

- Functional diversity metrics include Richness (`FRic`), Evenness (`FEve`), 
Divergence (`FDiv`), Dispersion (`FDis`) and Rao entropy  (`FRaoq`). 
These functional diversity metrics are computed with the package 
[`fundiversity`](https://funecology.github.io/fundiversity/)

The default parameterization includes computation of `shannon` and `bray`. 

Functional diversity metrics require significantly larger computational time 
than $\alpha$ and $\beta$ diversity metrics. 

__Therefore, we recommend computing functional metrics for validation, or to map them over a limited spatial extent when exploring the package.__
...
The function `biodivMapR` requires 
...
ab_info_SPCA <- biodivMapR(input_raster_path = PCA_Output$PCA_Files$PCA, 
...
`ab_info_SPCA` produced from the `biodivMapR` function includes two lists:
...
These files can then be used for an independent process with `biodivMapR`. 
...
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/biodivMapR_vignette_illustrations/03_shannon.gif"
knitr::include_graphics(url)
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/biodivMapR_vignette_illustrations/03_beta.gif"
knitr::include_graphics(url)
```
...
`biodivMapR` can use the path for the vrt as `input_raster_path`. 
...
# 4- apply biodivMapR 
options <- set_options_biodivMapR(fun = 'biodivMapR_tiling')
options$nb_clusters <- nb_clusters
options$alpha_metrics <- c('richness', 'shannon', 'hill')

ab_info_SI <- biodivMapR_tiling(feature_dir = output_dir_si, 
...
```{r, eval=TRUE, echo = FALSE, out.width="100%"}
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/biodivMapR_vignette_illustrations/03_shannon_si.gif"
knitr::include_graphics(url)
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/biodivMapR_vignette_illustrations/03_beta_si.gif"
knitr::include_graphics(url)
```
<!-- <p> -->
