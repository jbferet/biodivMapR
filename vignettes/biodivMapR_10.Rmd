...
* <span style="color:red"> *FG : open subalpine grasslands dominated by Festuca violacea, mostly found on south-facing steep slopes* </span>.
* <span style="color:yellow"> *FP : tall subalpine grasslands dominated by Patzkea paniculata (syn. Festuca paniculata), mostly found on south-facing gentle slopes with deep soil* </span>.
* <span style="color:orange"> *SC : sparsely vegetated grasslands dominated by Sesleria caerulea on south-facing, debris-covered slopes* </span>.
* <span style="color:blue"> *V: low stature heaths dominated by Vaccinium uliginosum and Vaccinium myrtillus, mostly found on north-facing slopes* </span>.
...
<p>
  <img align="left" width="48%" height="48%" src="../man/figures/Lautaret/FP_001.JPG" alt = "FP1" title = "FP1">
  <img align="right" width="48%" height="48%" src="../man/figures/Lautaret/FP_002.JPG" alt = "FP2" title = "FP2">
</p>

<p>
  <img align="left" width="48%" height="48%" src="../man/figures/Lautaret/V_001.JPG" alt = "FP1" title = "FP1">
  <img align="right" width="48%" height="48%" src="../man/figures/Lautaret/V_002.JPG" alt = "FP2" title = "FP2">
</p>

<p>
  <img align="left" width="48%" height="48%" src="../man/figures/Lautaret/FV_001.JPG" alt = "FP1" title = "FP1">
  <img align="right" width="48%" height="48%" src="../man/figures/Lautaret/S_001.JPG" alt = "FP2" title = "FP2">
</p>
...
NDVI <- ComputeSpectralIndices_fromExpression(Refl = ImBrick, SensorBands = SensorBands, 
                                               ...
                                               ReflFactor=ReflFactor, NameIndex = 'NDVI')
...
TCARI <- ComputeSpectralIndices_fromExpression(Refl = ImBrick, SensorBands = SensorBands,
                                               ExpressIndex = Formula_TCARI, listBands = Bands_TCARI,
                                               ReflFactor=ReflFactor,NameIndex = 'TCARI')
...
# 3- application of spectral index on image
OSAVI <- ComputeSpectralIndices_fromExpression(Refl = ImBrick, SensorBands = SensorBands,
                                               ExpressIndex = Formula_OSAVI, listBands = Bands_OSAVI,
                                               ReflFactor=ReflFactor,NameIndex = 'OSAVI')
...
#############################       CR_SWIR     ##################################
# 1- The expression for computation of continuum removal is already coded in PROSAIL
# 2- Define the spectral bands corresponding to bands in the spectral index, identified as BXX, with XX the number of the band
Bands_CR_SWIR <- list()
...
# 3- Compute the spectral indices
CR_SWIR <- CR_WL(Refl = ImBrick, SensorBands = SensorBands,
                 CRbands = Bands_CR_SWIR, ReflFactor=ReflFactor)
CR_SWIR <- CR_SWIR[[1]]
...
# 3- Compute the spectral indices
ND_LMA <- ComputeSpectralIndices_fromExpression(Refl = ImBrick, SensorBands = SensorBands, 
                                               ExpressIndex = Expression_ND_LMA, listBands = Bands_ND_LMA, 
                                               ReflFactor=ReflFactor, NameIndex = 'ND_LMA')
...
AUCminmax[['CRmin']] <- 650
AUCminmax[['CRmax']] <- 720
ANCB <- AUC(Refl = ImBrick, SensorBands = SensorBands, AUCminmax = AUCminmax, ReflFactor=ReflFactor)
AUCminmax[['target']] <- 670
ANCB <- ANCB/(1-CR_WL(Refl = ImBrick, SensorBands = SensorBands,
                      CRbands = AUCminmax, ReflFactor=ReflFactor)[[1]])
```
...
write_StarsStack(StarsObj = StarsObj, dsn = Path_StackIndices, BandNames =names(Index_Path), datatype='Float32')
```
