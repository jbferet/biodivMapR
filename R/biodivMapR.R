...
#' - beta_metrics list. beta diversity metrics: bray, brayturn, simpson_diss, jaccard, jaccardturn, sorensen
...
biodivMapR <- function(
  ...
  # define options
  options <- set_options_biodivMapR(fun = 'biodivMapR', options = options)
  ...
  if (is.null(Beta_info_save) & compute_beta)
  ...
  if (moving_window){
    ...
    betanames <- paste0(beta_metrics,'_mw')
