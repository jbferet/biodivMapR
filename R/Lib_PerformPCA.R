...
#' @param FilterPCA 2nd filtering based on PCA
#' @param Excluded.WL Water Vapor Absorption domains (in nanometers). Can also be used to exclude spectific domains
#' @param NbIter number of iteration to estimate diversity from the raster
#' @param nbCPU number fo CPUs in use
#' @param MaxRAM max number of RAM allocated during data read (may require more RAM during data process)
#'
#' @return PCA.Files is a list of paths corresponding to resulting PCA files
Perform.PCA.Image = function(ImPath,ImPathShade,Output.Dir,Continuum.Removal=TRUE,TypePCA='SPCA',FilterPCA=FALSE,Excluded.WL=FALSE,NbIter=20,nbCPU=1,MaxRAM = 0.25){
  ...
  if (FilterPCA==TRUE){
    ...
    if (size(PCA.model$dataPCA)[2]>5){
      PCsel     = 1:5
    } else {
      PCsel     = 1:size(PCA.model$dataPCA)[2]
    }
  ...
  Nb.PCs  = size(PCA.model$dataPCA)[2]
  if (Nb.PCs>30){
    Nb.PCs   = 30
  }
...
Filter.PCA = function(ImPath,HDR,ImPathShade,Shade.Update,Spectral,CR,PCA.model,PCsel,TypePCA,nbCPU = 1,MaxRAM=0.25){
  ...
  fidShade.Update   =file(description = Shade.Update, open = "wb", blocking = TRUE,
  ...
  for (i in 1:nbPieces){
    ...
    # files to write in
    fidOUT        = file(description = Shade.Update, open = "r+b", blocking = TRUE,
                         encoding = getOption("encoding"), raw = FALSE)
...
Create.PCA.Image = function(ImPath,ImPathShade,PCA.Path,PCA.model,Spectral,Nb.PCs,CR,TypePCA,nbCPU=1,MaxRAM= 0.25){
  ...
  # create updated shade mask
  fidPCA    = file(description = PCA.Path, open = "wb", blocking = TRUE,
                   encoding = getOption("encoding"), raw = FALSE)
  close(fidPCA)
  ...
  for (i in 1:nbPieces){
    ...
    # files to write in
    fidOUT        = file(description = PCA.Path, open = "r+b", blocking = TRUE,
                         encoding = getOption("encoding"), raw = FALSE)
