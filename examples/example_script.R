...
PCA_Output <- perform_PCA(Input_Image_File, Input_Mask_File, Output_Dir, TypePCA = TypePCA,
                          FilterPCA = FilterPCA, nbCPU = nbCPU, MaxRAM = MaxRAM, Continuum_Removal = TRUE)
...
map_spectral_species(Input_Image_File, Output_Dir, PCA_Files, PCA_model,
                     SpectralFilter, Input_Mask_File, Pix_Per_Partition,
                     nb_partitions, nbCPU = nbCPU, MaxRAM = MaxRAM,
...
write.table(Shannon.RS, file = paste(Path.Results, "ShannonIndex.csv", sep = ""),
            sep = "\t", dec = ".", na = " ", row.names = Biodiv.Indicators$Name_Plot,
